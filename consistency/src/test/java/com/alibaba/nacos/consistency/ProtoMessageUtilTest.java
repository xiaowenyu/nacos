...
public class ProtoMessageUtilTest {
    ...
    public void testParseWriteRequestWithKey() {
        String group = "test";
        String key = "testKey";
        ReadRequest testCase = ReadRequest.newBuilder().setGroup(group).setData(ByteString.copyFrom(key.getBytes())).build();
        Object actual = ProtoMessageUtil.parse(testCase.toByteArray());
        Assert.assertEquals(ReadRequest.class, actual.getClass());
    }
